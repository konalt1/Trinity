...
- Прекеш: `Precache()` в `addon_game_mode.lua` вызывает `PrecacheUnitByNameSync` для `npc_mortimer_boss` и `npc_mortimer_boss_finale`. Этого достаточно: юнит тянет `Model`, `GameSoundsFile`, `VoiceFile`, `ProjectileModel` и все способности из KV, включая ванильный `snapfire_firesnap_cookie`. Отдельно перечислять `.vpcf` Снапфайра не нужно, `particle_folder` на `hero_snapfire` не возвращать — там 239 файлов, из которых около сотни принадлежат Scatterblast, Lil' Shredder, loadout и скипетровым версиям.

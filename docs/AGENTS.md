...
| `caravan_spear_debug 0\|1` | Отрисовка и лог копий Аганима (слоты, прицел, полёт, попадание, пригвождение) |
...
- Модель Аганима — Labyrinth Apex Mage: `models/heroes/aghanim/aghanim_model.vmdl`. Лазер — `particles/creatures/aghanim/staff_beam*`. Осколки — ненаправленные linear снаряды в сторону героев на момент каста. Копья — модель `models/heroes/mars/mars_spear.vmdl`, импакт — `mars_spear_impact.vpcf`. Анимации: `cast2` / channel для лазера, `cast_shard_attack` для осколков, `attack_cast1` для копий.
...
- Курьеры идут прямоугольником относительно взгляда Аганима: 3 впереди, 3 сзади, 2 слева и 2 справа (~380 вперёд/назад и в стороны). `ModelScale` курьеров — `1.3`. Поводок ~1100. Если курьер дальше своего слота чем ~120, он ускоряется до **280** и идёт на слот, пока не подойдёт ближе ~80; во время разбега ускорение не действует. Удар по любому курьеру заставляет всю пачку разбежаться от обидчика; поводок не телепортирует и не лечит.
- Пока пачка жива, обе команды постоянно видят Аганима и область ~1800 вокруг него (`modifier_caravan_global_vision`).
...
- Аганим по очереди кастует лазер, осколки и копья с паузой ~2 сек. Лазерный linger уничтожается (~0.65 с и при конце каста). Вокруг Аганима появляются модели `mars_spear.vmdl`; каждое копьё целится в ближайшего героя, повтор цели разрешён. Через **1** сек направление фиксируется, копьё летит как у Марса: тащит первого героя, застревает в дереве, постройке или рельефе и оглушает жертву на копье. Урон `200/400/600`, дальность `900/1000/1100`, стан `1,3/1,6/1,9` по стадии каравана. Попадает в невидимых и невосприимчивых к магии. Debug: `caravan_spear_debug 1` рисует слоты, прицел, коридор полёта и точки попадания/пригвождения и пишет события в консоль; `caravan_spear_debug 0` отключает.

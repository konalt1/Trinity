...
- Окно лутбоксов почти на весь экран: края затемнены, по центру картинка сокровищницы с числом неоткрытых боксов и названием «Сокровищница стикеров», внизу горизонтальный ряд всего каталога. Открытие — клик по сундуку или кнопка «Открыть». Сервер сразу роллит награду; клиент показывает горизонтальную рулетку точно по вертикальному центру окна, а сундук и нижний каталог на время спина скрываются. Лента набирается по тем же весам, что и дроп; элитные слоты с золотой рамкой. Спин длится около 9 секунд: первые 5 секунд лента идёт с высокой постоянной скоростью, последние 4 секунды плавно замедляется до полной остановки. Клик по ленте, крестик, `Esc` или затемнение во время спина сразу показывают результат. После остановки лента уходит, нижний каталог возвращается, а в центре показываются большое видео стикера, имя, «Обычный»/«Элита» и «Дальше». Закрытие после reveal — крестик, клик по затемнению, `Esc`. Наведение на стикер в ряду всегда играет его видео и звук; под стикером число копий или прочерк, элита — золотая рамка

...
| `caravan_retreat_debug 0\|1` | Лог и линия отхода Аганима после смерти всех курьеров (по умолчанию включён) |
...
- Окно стикеров модальное: затемнение на весь HUD, слева колесо из 8 слотов по радиусу вокруг нативной графики центра (`s2r://panorama/images/chat_wheel/`), справа только открытые стикеры, которые ещё не стоят на колесе. Закрытие — крестик, клик по затемнению, `Esc`
...
- Слоты колеса визуально стоят через `transform`; клик и дроп выбирают слот по курсору относительно центра `WheelArea`. В `trinity_sticker_save_wheel` уходят `PlayerID` и `s0`–`s7` (пустой слот = `-`). Один стикер не может стоять в двух слотах. Подписи боевого чат-колеса тоже позиционируются `transform`, не `margin`
...
- Вскрытие: −1 бокс, +1 валюта этого бокса, один ролл по весам наград. Элитный вес = 1/10 нормального веса того же стикера. Частые стикеры: Gura, NeuroHug, Watson, Anime, Choso (вес 100/10). Редкие: Neurodance, StickerOne, StickerTwo (30/3)
...
- Для каждого стикера `CHAT_STICKER_SOUNDS` в `chat_wheel.js` задаёт существующее однократное sound event Dota. Звук запускается только у элитной версии; нормальная показывает то же видео без звука. Элита помечается золотой рамкой и ★
...
Первый ивент — караван на **0:10**, дальше каждые **5 минут** от момента спавна, пока слот не занят. Очередь: караван → Мортимер → караван → Мортимер. Стадия каравана и уровень Мортимера считаются раздельно; стадия каравана ограничена 3. Спавн только после рога (`GAME_IN_PROGRESS`), не во время разминки.

#### Караван курьеров

- Неуязвимый `npc_caravan_aghanim` ведёт пачку из **10 уникальных** курьеров, случайно выбранных из пула 13. `ModelScale` Аганима — `1.65`. Полосы HP и маны скрыты. На выход играет `Caravan.Spawn` (`sounds/ui/ui_findmatch_search_01.vsnd`).
...
- Курьеры умирают за **1 / 2 / 3 атаки** по стадии Аганима, как вард: способности хиты не снимают. Пока Аганим стоит на месте, курьеры держат слот и idle, без дёрганья за поворотом каста.
...
- На точке выхода Мортимера (спавнер / `Roshan_pathway`) пачка исчезает без дропа. У курьера в инвентаре видны предметы стадии и мешок золота (заряды = суммарное золото мешков). Убитый игроками курьер бросает эти предметы на землю (`LaunchLoot`) и мешки `item_caravan_gold_bag` по `50G` × `1/2/4` (Golden Flopjaw — `4/8/16`). Huntling на 3 стадии бросает только мешки, без Tranquil Boots.
- Аганим по очереди кастует копья, осколки и лазер с паузой ~2 сек, если герой в радиусе карусели + 200 (сейчас **1000** при `orbit_radius` 800). После каста осколков Аганим идёт дальше: вокруг него крутятся **10** кристаллов (радиус 800, касание 80, 35°/сек), пока все не разобьются о героев. Касание живого героя ломает осколок: магический урон `140/220/300` и замедление хода и атаки `20/25/30` на `1,0/1,2/1,4` по стадии. Замедление стакается отдельным экземпляром на каждый осколок. BKB и иллюзии осколок не ломают; один герой может словить несколько. Повтор осколков пропускается, пока жив предыдущий набор. Вокруг Аганима появляются **3** модели `lich_ti8_immortal_ice_shards.vmdl` (масштаб 7); каждое копьё целится в ближайшего героя, повтор цели разрешён. Через **1** сек направление фиксируется, копьё летит как у Марса: тащит первого героя, застревает в дереве, постройке или рельефе и оглушает жертву на копье. Урон `200/400/600`, дальность `900/1000/1100`, стан `1,3/1,6/1,9` по стадии каравана. Попадает в невидимых и невосприимчивых к магии. Лазер — канальный луч в героев в радиусе способности: замедление, слепота и растущий урон. Debug копий: `caravan_spear_debug 1` (по умолчанию выключен).
...
Таблицы: `stickers`, `player_stickers` (`quality`, `copies`), `player_wheel`; у `players` — `lootbox_unopened`, `lootbox_currency`, `lootbox_grants_date`, `lootbox_grants_today`, `lootbox_daily_date`. Каталог сидится из PHP (`Gura` … `StickerTwo`).

...
function modifier_doom_scorched_earth_lua:OnCreated( kv )
	-- references
	self.base_damage = self:GetAbility():GetSpecialValueFor( "damage_per_second" )
	...
	-- precache damage table (damage will be calculated dynamically)
	self.damageTable = {
		-- victim = target,
...
function modifier_doom_scorched_earth_lua:OnRefresh( kv )
	...
	if not IsServer() then return end
	if not self.owner then return end
	-- damage will be calculated dynamically in OnIntervalThink
end
